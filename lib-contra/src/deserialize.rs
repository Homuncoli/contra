...
use std::{io::{self, ErrorKind}, marker::PhantomData, collections::HashMap, hash::Hash};
...
macro_rules! decl_visit_primitive {
    ($ttype: ident, $visit_fn: ident) => {
        fn $visit_fn(self, _v: $ttype) -> 
            Result<Self::Value, AnyError> { Err(visiting_but_expected(stringify!($ttype), &self.expected_a()).into()) }
    };
}
...
pub trait Visitor: Sized {
    ...
    fn visit_map<M: MapAccess>(self, _map: M) -> Result<Self::Value, AnyError> { Err(visiting_but_expected("map", &self.expected_a()).into()) } 
...
impl<I: Deserialize> Deserialize for Vec<I> {
    fn deserialize<D: Deserializer>(des: D) -> Result<Self, AnyError> {
        struct VecVisitor<I> { marker: PhantomData<I> }
        ...
        des.deserialize_seq(VecVisitor { marker: PhantomData::<I> })
    }
}

impl<K: Deserialize + Hash + Eq, V: Deserialize> Deserialize for HashMap<K, V> {
    fn deserialize<D: Deserializer>(des: D) -> Result<Self, AnyError> {
        struct HashMapVisitor<K, V> { k_marker: PhantomData<K>, v_marker: PhantomData<V> }
        ...
        des.deserialize_map(HashMapVisitor { k_marker: PhantomData, v_marker: PhantomData })
    }
}
